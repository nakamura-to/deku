(function (deku, parser) {
  var compiler = (function () {
    ...
    Compiler.prototype = {
      ...
      pushOpcode: function (name, param1, param2, param3) {
      ...
      pipeline: function (node) {
        ...
        var path = name.path;
        var processor;
        this.type_name(name, 'tmpContext', 'tmp');
        this.pushOpcode('op_evaluateValue', path);
        this.pushOpcode('op_applyPrePipeline', path);
        for (i = 0; i < len; i++) {
          processor = processors[i];
          this.type_name(processor, 'processorContext', 'processor');
      ...
      type_partial: function (node) {
        ...
        if (context) {
          this.type_name(context, 'tmpContext', 'tmp');
        } else {
          this.type_name({path: '$this', segments: ['$this']}, 'tmpContext', 'tmp');
        }
      ...
      type_name: function (node, context, local) {
        var segments = node.segments;
        var i;
        var len = segments.length;
        this.pushOpcode('op_lookupFromContext', segments[0], context, local);
        for (i = 1; i < len; i++) {
          this.pushOpcode('op_lookupFromTmp', segments[i], context, local);
        }
      }
    };
    ...
    JsCompiler.prototype = {
      ...
      op_lookupFromContext: function (name, localContext, local) {
      ...
      op_lookupFromTmp: function (name, localContext, local) {

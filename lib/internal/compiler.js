(function (deku, parser) {
  var compiler = (function () {
    ...
    Compiler.prototype = {
      ...
      defaultPartialContext: {path: '@this', segments: ['@this']},

      type_partial: function (node) {
        var name = node.name;
        var context = node.context || this.defaultPartialContext;
        ...
        if (name.charAt(0) === '@') {
          switch (name) {
            ...
            default:
              match = /^\@(\d+)/.exec(name);
              if (match) {
                this.source.push(local + ' = contextStack[contextStack.length - 1 - ' + match[1] + '];');
              } else {
